...
CREATE POLICY "Org members can insert price history" ON public.inventory_price_history FOR INSERT TO authenticated WITH CHECK (organization_id = get_user_org_id());
CREATE POLICY "Org members can view price history" ON public.inventory_price_history FOR SELECT TO authenticated USING (organization_id = get_user_org_id());
...
CREATE POLICY "Users can insert own profile" ON public.profiles FOR INSERT TO authenticated WITH CHECK ((user_id = auth.uid()) AND (organization_id = get_user_org_id()));
...
CREATE POLICY "Org members can insert templates" ON public.repair_templates FOR INSERT TO authenticated WITH CHECK ((organization_id = get_user_org_id()) AND ((created_by = auth.uid()) OR (created_by IS NULL)) AND ((is_public = false) OR has_role(auth.uid(), 'admin'::app_role)));
CREATE POLICY "Org members can update own templates" ON public.repair_templates FOR UPDATE TO authenticated USING (organization_id = get_user_org_id()) WITH CHECK ((organization_id = get_user_org_id()) AND ((created_by = auth.uid()) OR has_role(auth.uid(), 'admin'::app_role)) AND ((is_public = false) OR has_role(auth.uid(), 'admin'::app_role)));
