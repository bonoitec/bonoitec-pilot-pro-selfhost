...
-- Must drop and recreate each one
-- =============================================
...
CREATE POLICY "Admins can update organization" ON public.organizations FOR UPDATE TO authenticated USING ((id = get_user_org_id()) AND has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can insert organization" ON public.organizations FOR INSERT TO authenticated WITH CHECK (true);
...
-- FIX 2: REPAIR_TEMPLATES - restrict is_public to admins only
