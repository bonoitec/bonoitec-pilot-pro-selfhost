...
-- This is critical: RESTRICTIVE policies without PERMISSIVE counterparts block all access
...
CREATE POLICY "Admins can delete notification templates" ON public.notification_templates FOR DELETE TO authenticated USING (organization_id = get_user_org_id() AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can insert notification templates" ON public.notification_templates FOR INSERT TO authenticated WITH CHECK (organization_id = get_user_org_id() AND has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can update notification templates" ON public.notification_templates FOR UPDATE TO authenticated USING (organization_id = get_user_org_id() AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can insert organization" ON public.organizations FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Org members can insert templates" ON public.repair_templates FOR INSERT TO authenticated WITH CHECK (organization_id = get_user_org_id() AND (is_public = false OR has_role(auth.uid(), 'admin')));
CREATE POLICY "Org members can update own templates" ON public.repair_templates FOR UPDATE TO authenticated USING (organization_id = get_user_org_id()) WITH CHECK (organization_id = get_user_org_id() AND (is_public = false OR has_role(auth.uid(), 'admin')));
...
CREATE POLICY "Admins can manage technicians" ON public.technicians FOR ALL TO authenticated USING (organization_id = get_user_org_id() AND has_role(auth.uid(), 'admin')) WITH CHECK (organization_id = get_user_org_id() AND has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can manage roles in own org" ON public.user_roles FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin') AND EXISTS (SELECT 1 FROM profiles p1 WHERE p1.user_id = auth.uid() AND p1.organization_id = (SELECT p2.organization_id FROM profiles p2 WHERE p2.user_id = user_roles.user_id LIMIT 1)));
