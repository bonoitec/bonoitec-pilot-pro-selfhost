...
export function CreateRepairWizard({ open, onOpenChange }: Props) {
            ...
            body: {
              ...
              data: {
                ...
                estimatedDelay: (() => {
                  ...
                  if (!etm || etm <= 0) return "";
                  if (etm < 60) return `Délai estimé : environ ${etm} minutes`;
                  if (etm < 120) return `Délai estimé : environ 1 heure${etm % 60 > 0 ? ` ${etm % 60} min` : ""}`;
                  return `Délai estimé : environ ${Math.floor(etm / 60)} heures`;
                })(),
              },
  ...
  const handleClose = () => {
    ...
    setShowDiagnostic(false); setDiagnosticResult(null);
    onOpenChange(false);
  };
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const serialInfo = device.serial_number ? `Numéro de série / IMEI: ${device.serial_number}` : "";
                              const checkedItems = Object.entries(checklist).filter(([, v]) => v).map(([k]) => k);
                              const conditionInfo = `État physique — Écran: ${screenCondition}/5, Châssis: ${frameCondition}/5, Dos: ${backCondition}/5`;
                              const fullContext = [
                                `Appareil: ${deviceInfo}`,
                                serialInfo,
                                checkedItems.length > 0 ? `Checklist prise en charge: ${checkedItems.join(", ")}` : "",
                                conditionInfo,
                                repairType ? `Type de réparation: ${repairType}` : "",
                                `Description du problème: ${issue.trim()}`,
                              ].filter(Boolean).join("\n");
